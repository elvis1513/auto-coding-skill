...
<!-- auto-coding-skill:managed-workflow:start version=4.3.7 -->
...
`docs/project/auto-coding-skill.yaml`; the runtime overlays that file on these
defaults without allowing it to weaken code-enforced safety invariants.
...
- `risk`: default signals that project-specific rules may replace in the
  project overlay; code-enforced high-risk classifications remain mandatory.
- `structure` and `optimization`: managed defaults that the project overlay may
  replace explicitly.
...
Do not edit this file for project specialization. Run
`python3 docs/tools/autopipeline/ap.py config-effective --json` to inspect the
redacted effective configuration, and edit only the project overlay. Do not put
competing delivery flows, agent roles, lifecycle rules, gate semantics, or
completion rules outside this managed block.
<!-- auto-coding-skill:managed-workflow:end -->
# Project Facts Location
Keep durable project facts in the installed `docs/project/` files. Initialization
owns this document byte-for-byte and never rewrites an existing project overlay.
