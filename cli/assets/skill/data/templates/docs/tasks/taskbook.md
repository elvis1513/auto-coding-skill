...
2) 普通小改动只要求最小设计记录；跨模块、接口、数据库、部署、CI/Jenkins、关键页面流程变更才补 DD
3) 每个任务最终必须有闭环证据：提交、已启用的 CI/Jenkins 状态、已启用的目标环境验证、结果、遗留问题
...
- 非目标（Out of scope）：
- 是否高风险（Yes/No）：
- 高风险分类（如有）：DB / Auth / Payment / CI / Jenkins / Gateway / Upload / Prod Config
- 影响面（API / DB / CI / Jenkins / Page / Config / Deploy）：
...
- 目标环境验证：<health/api/page/business path or skipped by verification.target_env_required=false>
